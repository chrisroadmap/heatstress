...
"""
This script calculates the monthly means for all models and scenarios
"""
...
for model in scenarios:
    ...
    for run in scenarios[model]:
        for scenario in scenarios[model][run]:
            ...
            outdir = '/gws/pw/j05/cop26_hackathons/bristol/project10/utci_projections_1deg_monthly_means/%s/%s/%s/' % (model, scenario, run)
            mkdir_p(outdir)
            ...
            for year in range(firstyear, lastyear):
                ...
                if model in ['BCC-CSM2-MR', 'MRI-ESM2-0']:
                    ...
                    # KACE is already well behaved
                iris.coord_categorisation.add_month_number(cube_model, 'time', name='month')
                result = cube_model.aggregated_by('month', iris.analysis.MEAN)
